% pop_ImportInstCFormat() - Import Instep .c Format average files.
%
% Usage:
...
function [EEG com] = pop_ImportInstCFormat(EEG, FileName, FilePath);
...
if nargin < 2
    ...
    [FileName, FilePath] = uigetfile('*.*','Open BVA .raw format file:','*.*');
...
com = sprintf('[EEG com] = pop_ImportBVARawFormat( %s, ''%s'', ''%s'' );',inputname(1), FileName, FilePath);
